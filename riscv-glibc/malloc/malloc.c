...
unsigned int DPT_H = 0;
unsigned int DPT_F = 0;

void weak_variable (*__dpt_sysmalloc_hook) (void *__arena_end) = NULL;
void weak_variable (*__dpt_scan_bitmap_hook) (void *__ptr) = NULL;

#ifdef DPT_QEMU
unsigned int ALLOC_NEW_ARENA = 0;

void *weak_variable (*__dpt_tagd_hook)
	(void *__ptr, size_t __tweak) = NULL;
void *weak_variable (*__dpt_xtag_hook)
	(void *__ptr) = NULL;
void weak_variable (*__dpt_cstr_hook)
	(void *__ptr, size_t __size) = NULL;
void weak_variable (*__dpt_cclr_hook)
	(void *__ptr) = NULL;
#endif
  ...
  if ((unsigned long) (size) >= (unsigned long) (nb + MINSIZE))
    {
      ...
      check_malloced_chunk (av, p, nb);
			...
			//yh+begin
			void (*hook) (void *)
				= atomic_forced_read (__dpt_sysmalloc_hook);

			if (__builtin_expect (hook != NULL, 0)) {
			#ifdef DPT_QEMU
				size_t alloc_new_arena = ALLOC_NEW_ARENA;
			#else
				size_t dpt_config = (size_t) csr_read(0x430 /* CSR_DPT_CONFIG */);
				//printf("[SYS-MALLOC] dpt_config: 0x%lx\n", dpt_config);
				size_t alloc_new_arena = ((dpt_config >> 54) & 0x1);
			#endif

				if (alloc_new_arena) {
					#define ROUND_UP ((size_t) 0x800000)
					old_top = av->top;
					old_size = chunksize (old_top);
					old_end = (char *) (chunk_at_offset (old_top, old_size));
          ...
          LIBC_PROBE (memory_sbrk_more, 2, brk, size);
            ...
            mchunkptr filler = (mchunkptr) ((size_t) arena_end - (size_t) 0x10);
						...
						size_t prev_size = (size_t) ((char *) filler - (char *) old_top);
						set_head (old_top, prev_size | PREV_INUSE | (av != &main_arena ? NON_MAIN_ARENA : 0));

						set_head (filler, MINSIZE | PREV_INUSE | (av != &main_arena ? NON_MAIN_ARENA : 0));

						remainder = chunk_at_offset (filler, MINSIZE);
						av->top = remainder;
						remainder_size = (size_t) (new_end - (char *) remainder);
  ...
  if (tc_idx < mp_.tcache_bins
      ...
      && tcache->counts[tc_idx] > 0)
    {
      victim = tcache_get (tc_idx);
			...
			if (DPT_H) {
			#ifdef DPT_QEMU
				void *(*tagd_hook) (void *, size_t)
					= atomic_forced_read (__dpt_tagd_hook);
				if (__builtin_expect (tagd_hook != NULL, 0))
					victim = (*tagd_hook)(victim, (size_t) victim);

				void (*cstr_hook) (void *, size_t)
					= atomic_forced_read (__dpt_cstr_hook);
				if (__builtin_expect (cstr_hook != NULL, 0))
					(*cstr_hook)(victim, bytes);
			#else
				asm (".insn r 0xb, 0x1, 0x0, %0, %1, %2" : "=r" (victim) : "r" (victim), "r" (victim)); // tagd
				asm (".insn s 0x2b, 0x3, %0, 0x0(%1)" : : "r" (bytes), "r" (victim)); // cstr
			#endif
			}
			//yh+end
...
__libc_free (void *mem)
{
  ...
  if (mem == 0)                              /* free(0) has no effect */
	...
	if (DPT_H) {
		if (DPT_F)
			__dpt_scan_bitmap_hook(mem);
	#ifdef DPT_QEMU
		void (*cclr_hook) (void *)
			= atomic_forced_read (__dpt_cclr_hook);
		if (__builtin_expect (cclr_hook != NULL, 0))
			(*cclr_hook)(mem);

		void *(*xtag_hook) (void *)
			= atomic_forced_read (__dpt_xtag_hook);
		if (__builtin_expect (xtag_hook != NULL, 0))
			mem = (*xtag_hook)(mem);
	#else
		asm (".insn s 0x2b, 0x7, x0, 0x0(%0)" : : "r" (mem)); // cclr
		asm (".insn r 0xb, 0x4, 0x0, %0, %1, x0" : "=r" (mem) : "r" (mem)); // xtag
	#endif
	}
	//yh+end
...
#ifdef DPT_QUARANTINE_ENABLED
  quarantine_insert(mem);
  return;
#endif
  ...
  if (oldmem == 0)
	...
	if (DPT_H) {
		...
		void *(*xtag_hook) (void *)
			= atomic_forced_read (__dpt_xtag_hook);
		if (__builtin_expect (xtag_hook != NULL, 0))
			oldmem = (*xtag_hook)(oldmem);
	#else
		asm (".insn s 0x2b, 0x7, x0, 0x0(%0)" : : "r" (oldmem)); // cclr
		asm (".insn r 0xb, 0x4, 0x0, %0, %1, x0" : "=r" (oldmem) : "r" (oldmem)); // xtag
	#endif
	}
	//yh+end
	  ...
	  void *newmem = chunk2rawmem (newp);
		...
		if (DPT_H) {
			...
			void *(*tagd_hook) (void *, size_t)
				= atomic_forced_read (__dpt_tagd_hook);
			if (__builtin_expect (tagd_hook != NULL, 0))
				newmem = (*tagd_hook)(newmem, (size_t) newmem);

			void (*cstr_hook) (void *, size_t)
				= atomic_forced_read (__dpt_cstr_hook);
			if (__builtin_expect (cstr_hook != NULL, 0))
				(*cstr_hook)(newmem, bytes);
		#else
			asm (".insn r 0xb, 0x1, 0x0, %0, %1, %2" : "=r" (newmem) : "r" (newmem), "r" (newmem)); // tagd
			asm (".insn s 0x2b, 0x3, %0, 0x0(%1)" : : "r" (bytes), "r" (newmem)); // cstr
		#endif
		}
		//yh+end
	      ...
	      ar_ptr == arena_for_chunk (mem2chunk (newp)));
			...
			if (DPT_H) {
				...
				void *(*tagd_hook) (void *, size_t)
					= atomic_forced_read (__dpt_tagd_hook);
				if (__builtin_expect (tagd_hook != NULL, 0))
					newp = (*tagd_hook)(newp, (size_t) newp);

				void (*cstr_hook) (void *, size_t)
					= atomic_forced_read (__dpt_cstr_hook);
				if (__builtin_expect (cstr_hook != NULL, 0))
					(*cstr_hook)(newp, bytes);
			#else
				asm (".insn r 0xb, 0x1, 0x0, %0, %1, %2" : "=r" (newp) : "r" (newp), "r" (newp)); // tagd
				asm (".insn s 0x2b, 0x3, %0, 0x0(%1)" : : "r" (bytes), "r" (newp)); // cstr
			#endif
			}
			//yh+end
...
# endif
  ...
  /* If we are using memory tagging, then we need to set the tags
	...
	//#else
	//	void *ptr_t;
	//	asm (".insn r 0xb, 0x1, 0x0, %0, %1, %2" : "=r" (ptr_t) : "r" (mem), "r" (mem)); // tagd
	//	asm (".insn s 0x2b, 0x3, %0, 0x0(%1)" : : "r" (sz), "r" (ptr_t)); // cstr
	//#endif
	//}
	////yh+end
      ...
      //yh-  return memset (mem, 0, sz);
			...
			if (DPT_H) {
				...
				if (__builtin_expect (cstr_hook != NULL, 0))
					(*cstr_hook)(mem, sz);
			#else
				asm (".insn r 0xb, 0x1, 0x0, %0, %1, %2" : "=r" (mem) : "r" (mem), "r" (mem)); // tagd
				asm (".insn s 0x2b, 0x3, %0, 0x0(%1)" : : "r" (sz), "r" (mem)); // cstr
			#endif
			}
			//yh+end
  ...
  if (nclears > 9)
  //yh-  return memset (d, 0, clearsize);
	//yh+begin
	{
		d = memset (d, 0, clearsize);
		...
		if (DPT_H) {
			...
			if (__builtin_expect (cstr_hook != NULL, 0))
				(*cstr_hook)(d, sz);
		#else
			asm (".insn r 0xb, 0x1, 0x0, %0, %1, %2" : "=r" (d) : "r" (d), "r" (d)); // tagd
			asm (".insn s 0x2b, 0x3, %0, 0x0(%1)" : : "r" (sz), "r" (d)); // cstr
		#endif
		}
...
#ifdef DPT_QUARANTINE_ENABLED
void quarantine_free(int qb_widx) {
  void *mem;
  mstate ar_ptr;
  mchunkptr p;                          /* chunk corresponding to mem */

  for (int i=0; i<QDEPTH; i++) {
    mem = quarantine_buffers[qb_widx][i];
    if (mem == NULL) {
      //printf("Skip mem == NULL! qb_widx: %d qb_didx: %d\n", qb_widx, i);
      continue;
    }
    ...
    //printf("Free quarantined pointer: %p qb_widx: %d qb_didx: %d\n", p, qb_widx, i);
  }
}

void quarantine_insert(void *mem) {
  //printf("Quarantine pointer: %p qb_widx: %d qb_didx: %d\n", mem, qb_widx, qb_didx);
  quarantine_buffers[qb_widx][qb_didx] = mem;

  qb_didx = ((qb_didx + 1) % QDEPTH);

  if (qb_didx == 0) {
    qb_widx = ((qb_widx + 1) % QWIDTH);
    quarantine_free(qb_widx);
  }
}
#endif
//yh+end
